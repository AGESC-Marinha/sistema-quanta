...
// CONFIGURAÇÃO DIRETA (SEM ERRO DE ENV)
...
export default function App() {
  ...
  async function fetchCondominios() {
    try {
      setLoading(true);
      // REMOVIDO O .order('created_at') QUE CAUSAVA O ERRO
              ...
              <div className="bg-white p-8 rounded-3xl shadow-sm border border-slate-200">
                ...
                <form onSubmit={handleSubmit} className="grid grid-cols-1 md:grid-cols-3 gap-6">
                  ...
                  <div className="md:col-span-3 flex gap-4">
                    <button className="flex-1 bg-blue-900 text-white p-5 rounded-2xl font-black text-lg hover:bg-blue-800 transition-all">
                      {editingId ? 'SALVAR ALTERAÇÕES' : 'SALVAR CONDOMÍNIO'}
                    </button>
                    {editingId && <button type="button" onClick={() => {setEditingId(null); setForm({nome:'',endereco:'',cnpj:'',dados_bancarios:'',saldo_fundo_reserva:'',projetos_incendio:'',possui_elevadores:false,qtd_elevadores:'',empresa_elevadores:'',status_manutencao:'Em dia',qtd_pnr:'',qtd_civis:'',despesa_estimada:''})}} className="bg-slate-200 text-slate-600 px-8 rounded-2xl font-black">CANCELAR</button>}
                  </div>
                </form>
              </div>
          ...
          <div className="bg-white rounded-3xl shadow-2xl w-full max-w-2xl overflow-hidden" onClick={e => e.stopPropagation()}>
            ...
            <div className="p-8 grid grid-cols-1 md:grid-cols-2 gap-6">
              ...
              <div className="md:col-span-2 border-t pt-4">
                ...
                <p className="font-bold">{selectedCondo.possui_elevadores ? `Sim (${selectedCondo.qtd_elevadores} und) - Empresa: ${selectedCondo.empresa_elevadores}` : 'Não possui'}</p>
                <p className="text-xs mt-1">Status: <span className="font-bold text-blue-600">{selectedCondo.status_manutencao}</span></p>
              </div>
              <div className="md:col-span-2 border-t pt-4">
